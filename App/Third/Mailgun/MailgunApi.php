...
class MailgunApi {
    ...
    {
        if (count($members) === 0) {
            throw new \Exception('The members list is empty.');
        }

        if (count($members) > 1000) {
            throw new \Exception('The maximum number of members allowed in one request is 1,000.');
        }
    ...
    protected function _performRequest($method, $url, MailgunObject $object = null, $params = array()){
        ...
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, $Config['ssl_verify']);
        ...
        if ($responseStatus >= 500) {
            throw new \Exception('Mailgun server error', $responseStatus);
        } else {
            $responseArray = json_decode($response, true);
            if ($responseStatus === 200) {
                return $responseArray;
            } else {
                throw new \Exception(!empty($responseArray['message']) ? $responseArray['message'] : $response,
                                           $responseStatus);
            }
        }
    }
