...
if (!function_exists("uploadImage")) {
    ...
    function uploadImage($input, $size = "5M", $pixels = null){
        global $Config;
        $storage = new \Upload\Storage\FileSystem(ROOT_PATH . $Config['upload']['image']);
...
if (!function_exists("imageCache")) {
    ...
    function imageCache($filename, $width, $height){
        ...
        echo ROOT_PATH. $Config['upload']['image'] . $filename;
        if (!is_file(ROOT_PATH. $Config['upload']['image'] . $filename)) {
            return null;
        }
        ...
        if (!is_file(ROOT_PATH. $Config['cache_image'] . $image_new) || (filectime(ROOT_PATH. $Config['upload']['image'] . $image_old) > filectime(ROOT_PATH. $Config['cache_image'] . $image_new))) {
            list($width_orig, $height_orig, $image_type) = getimagesize(ROOT_PATH. $Config['upload']['image'] . $image_old);

            if (!in_array($image_type, array(IMAGETYPE_PNG, IMAGETYPE_JPEG, IMAGETYPE_GIF))) {
                return ROOT_PATH. $Config['upload']['image'] . $image_old;
            }
            ...
            foreach ($directories as $directory) {
                $path = $path . '/' . $directory;
                if (!is_dir(ROOT_PATH. $Config['cache_image'] . $path)) {
                    @mkdir(ROOT_PATH. $Config['cache_image']  . $path, 0777);
                }
            }

            if ($width_orig != $width || $height_orig != $height) {
                $image = new \System\Libraries\Images(ROOT_PATH. $Config['upload']['image'] . $image_old);
                $image->resize($width, $height);
                $image->save(ROOT_PATH. $Config['cache_image'] . $image_new);
            } else {
                copy(ROOT_PATH. $Config['upload']['image'] . $image_old, ROOT_PATH. $Config['cache_image'] . $image_new);
            }
        }
